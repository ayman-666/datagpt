...
def buy(direction , duration = 15 ,ws2 = websocket.WebSocket(), martingale = True):
    ws2.connect(apiUrl)
...
def get_stream(ws10 = websocket.WebSocket()):
    ...
    try:
        ...
        #new_candle = (int(rec.iloc[[-1]]['epoch']) == int(time.time()))
        #print(f'current epoch {new_candle}\n')
        #print(f'current last candles \n {rec.tail()}')
...
def analize(df):
    ...
    new_candle = (int(time.time())) in range((int(win.iloc[[-1]]['epoch'])) , (int(win.iloc[[-1]]['epoch']) + 60))
    #new_candle = (int(win.iloc[[-1]]['epoch']) in range(int(time.time()),(int(time.time())+60)) )

    #print(f"current time {int(time.time())} ,,, new candle = {new_candle}")

    if (prev_close == min_close) and new_candle :
        acc = 1
    elif (prev_close == max_close) and new_candle:
        acc = 2 
